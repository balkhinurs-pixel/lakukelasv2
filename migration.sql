...
VALUES ('profile-images', 'profile-images', true, 1048576, ARRAY['image/jpeg', 'image/png', 'image/webp'])
...
-- 2. Allow authenticated users to upload with size limit
DROP POLICY IF EXISTS "Allow authenticated users to upload" ON storage.objects;
CREATE POLICY "Allow authenticated users to upload" ON storage.objects FOR INSERT TO authenticated WITH CHECK (
    bucket_id = 'profile-images' AND
    (storage.metadata(name) ->> 'size')::bigint < 1048576 -- 1MB general limit, can be more specific
);
